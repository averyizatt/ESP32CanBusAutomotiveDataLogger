...
// we reuse your existing CS pin from the main sketch
extern const int CAN_CS;
...
// forward declarations (Arduino only auto-prototypes .ino files)
void canRead();
void canValTracker();
...
// replacement for old WDT reset
void resetDevice() {
  ESP.restart();
}

/**
 * @brief CAN Commander setup, call once when entering hack mode
 */
void canCommanderSetup()
{
  ...
  // we assume Serial is already begun in your main sketch
  Serial.println();
  Serial.println("=== CAN Commander (ESP32 module) ===");
  ...
  while (currentMode == MODE_NONE)
  {
    ...
    // tiny yield so your main loop can still service WiFi if you call this from there
    delay(5);
  }
}

/**
 * @brief CAN Commander loop, call repeatedly while in hack mode
 */
...
void setupCanFilters()
{
  ...
  mcp2515.setFilterMask(MCP2515::MASK1, false, mask);
...
uint8_t setupPID()
{
  ...
  pid = strtoul(inputBuffer, NULL, 16);
...
void sendPidRequest(uint8_t pid)
{
  ...
  pidRequestFrame.can_id = 0x7DF;
  pidRequestFrame.can_dlc = 8;

  pidRequestFrame.data[0] = 0x02;
  pidRequestFrame.data[1] = 0x01;
  pidRequestFrame.data[2] = pid;
  for (int i = 3; i < 8; i++)
  {
  ...
  if (mcp2515.readMessage(&canMsg) == MCP2515::ERROR_OK)
  {
    if (canMsg.can_id >= 0x7E8 && canMsg.can_id <= 0x7EF)
    {
      if (canMsg.data[2] == pid)
      {
        ...
        {
          int rpmVal = ((canMsg.data[3] * 256) + canMsg.data[4]) / 4;
        ...
        {
          unsigned long odometerKm =
              ((unsigned long)(canMsg.data[3] == 0xCC ? 0 : canMsg.data[3]) << 24) |
