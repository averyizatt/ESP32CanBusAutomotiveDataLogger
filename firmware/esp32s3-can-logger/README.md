# ESP32-S3 CAN Logger — Firmware

The main production firmware for the ESP32 CAN Bus Automotive Data Logger, targeting the **ESP32-S3 DevKitC-1**.

Features:
- **CAN via MCP2515** (SPI) with live WebSocket frame broadcast
- **Web UI** served from SPIFFS — dashboard, CAN viewer, OTA, Wi-Fi config
- **Optional SD logging** (SPI microSD module, separate CS pin)
- **Optional ESP-NOW → CAN forwarding** (MAC allowlist, deny-by-default)
- **OLED display** (SSD1306 128×32, I²C) for quick status at a glance
- **RGB status LED** (WS2812-compatible, `RGB_BUILTIN`)
- **Watchdog + safe-mode** recovery for harsh automotive environments
- **Authenticated REST API** for remote control and configuration

→ See the [root README](../../README.md) for a full project overview, hardware bill of materials, and quick-start guide.  
